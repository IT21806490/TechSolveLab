...
async function generateZonesAndFares() {
  ...
  try {
    ...
    // Build route stop patterns to detect directions
    const routePatterns = {}
    const tripStops = {}
    ...
    stopTimes.forEach(st => {
      ...
      tripStops[tripId].push({ stopId, sequence })
    })
    ...
    const rows = fareAttrs.map(a => headers.map(h => a[h] || '').join(','))
  ...
  <div class="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 py-8 px-4">
    <div class="container mx-auto max-w-4xl">
      ...
      <div class="bg-white rounded-lg shadow-lg p-6 mb-6">
        ...
        <div class="bg-blue-50 border-l-4 border-blue-500 rounded-lg p-4 mb-4">
          <div class="flex items-start">
            ...
            <div class="flex-1">
              ...
              <p class="text-blue-700 text-sm mb-2">Upload these files to generate fare zones:</p>
              <div class="bg-white rounded p-2 font-mono text-sm border border-blue-200">
                <div class="text-blue-900">• <strong>stops.txt</strong> - Stop locations</div>
                <div class="text-blue-900">• <strong>trips.txt</strong> - Route mappings</div>
                <div class="text-blue-900">• <strong>stop_times.txt</strong> - Stop sequences</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="mb-4">
          <label class="block text-sm font-semibold text-gray-700 mb-2">stops.txt <span class="text-red-500">*</span></label>
          <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-blue-500 transition-colors">
            <input type="file" accept=".txt,.csv" @change="handleStops" id="stopsInput" class="hidden" />
        ...
        <div class="mb-4">
          <label class="block text-sm font-semibold text-gray-700 mb-2">trips.txt <span class="text-red-500">*</span></label>
          <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-blue-500 transition-colors">
            <input type="file" accept=".txt,.csv" @change="handleTrips" id="tripsInput" class="hidden" />
        ...
        <div class="mb-4">
          <label class="block text-sm font-semibold text-gray-700 mb-2">stop_times.txt <span class="text-red-500">*</span></label>
          <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-blue-500 transition-colors">
            <input type="file" accept=".txt,.csv" @change="handleStopTimes" id="stopTimesInput" class="hidden" />
      ...
      <div class="bg-white rounded-lg shadow-lg p-6 mb-6">
        <h2 class="text-xl font-semibold text-gray-800 mb-4">2. Configure Fare Settings</h2>
        <div class="mb-4">
          <label class="flex items-center cursor-pointer">
            <input type="checkbox" v-model="singleStage" class="w-5 h-5 text-blue-600 rounded focus:ring-blue-500 focus:ring-2" />
            <span class="ml-3 text-gray-700 font-medium">Assign entire route as single fare stage</span>
          </label>
          <p class="text-sm text-gray-500 ml-8 mt-1">Recommended for flat-fare systems</p>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            <label class="block text-sm font-semibold text-gray-700 mb-2">Fare ID</label>
            <input v-model="fareId" type="text" placeholder="FARE_NORMAL" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
          </div>
          <div>
            <label class="block text-sm font-semibold text-gray-700 mb-2">Fare Amount</label>
            <input v-model="fareAmount" type="number" step="0.01" placeholder="50.00" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
          </div>
          <div>
            <label class="block text-sm font-semibold text-gray-700 mb-2">Currency Code</label>
            <input v-model="currencyType" type="text" maxlength="3" placeholder="LKR" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent uppercase" />
          </div>
        </div>
      </div>
      ...
      <div class="bg-white rounded-lg shadow-lg p-6 mb-6">
        ...
        <button @click="generateZonesAndFares" :disabled="processing || !stopsFile || !tripsFile || !stopTimesFile" class="w-full bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-6 py-4 rounded-lg font-semibold hover:from-blue-700 hover:to-indigo-700 transition-all shadow-md hover:shadow-lg flex items-center justify-center space-x-2 disabled:opacity-50 disabled:cursor-not-allowed">
      ...
      <div v-if="outputStops" class="bg-white rounded-lg shadow-lg p-6 mb-6">
        <h2 class="text-xl font-semibold text-gray-800 mb-4">4. Review & Download Results</h2>
        <div class="bg-green-50 border-l-4 border-green-500 rounded-lg p-4 mb-4">
          <div class="flex items-start">
            ...
            <div class="flex-1">
              ...
              <p class="text-green-700 text-sm">Your GTFS fare files are ready for download</p>
            </div>
          </div>
        </div>
        ...
        <div class="space-y-3">
          <button @click="downloadFile(outputStops, 'stops.txt')" class="w-full bg-gradient-to-r from-green-600 to-emerald-600 text-white px-6 py-3 rounded-lg font-semibold hover:from-green-700 hover:to-emerald-700 transition-all shadow-md hover:shadow-lg flex items-center justify-center space-x-2">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path>
            </svg>
            <span>Download stops.txt (with zone_id)</span>
          </button>
          <button @click="downloadFile(outputFareRules, 'fare_rules.txt')" class="w-full bg-gradient-to-r from-green-600 to-emerald-600 text-white px-6 py-3 rounded-lg font-semibold hover:from-green-700 hover:to-emerald-700 transition-all shadow-md hover:shadow-lg flex items-center justify-center space-x-2">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path>
            </svg>
            <span>Download fare_rules.txt</span>
          </button>
          <button @click="downloadFile(outputFareAttributes, 'fare_attributes.txt')" class="w-full bg-gradient-to-r from-green-600 to-emerald-600 text-white px-6 py-3 rounded-lg font-semibold hover:from-green-700 hover:to-emerald-700 transition-all shadow-md hover:shadow-lg flex items-center justify-center space-x-2">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path>
            </svg>
        ...
        <button @click="reset" class="w-full mt-4 bg-gray-500 text-white px-6 py-3 rounded-lg font-semibold hover:bg-gray-600 transition-all shadow-md hover:shadow-lg">
      ...
      <div v-if="!outputStops && !processing" class="bg-white rounded-lg shadow-lg p-12 text-center">
        ...
        <h3 class="text-xl font-semibold text-gray-700 mb-2">Ready to Generate Fare Files</h3>
        <p class="text-gray-500">Upload your GTFS files to get started</p>
      </div>
    </div>
  </div>
</template>
